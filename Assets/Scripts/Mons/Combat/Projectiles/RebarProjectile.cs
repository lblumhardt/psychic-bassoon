...
public class RebarProjectile : MonoBehaviour
{
    ...
    {
        // A wall may first touch the side of the long bar. Check again as its tip advances.
        HandleTrigger(other);
    }
    ...
    private void HandleTrigger(Collider other)
    {
        ...
        {
            // The full-length trigger can overlap a wall well before the visible tip reaches it.
            // Only the wall directly ahead of the tip is a valid impact.
            Vector3 forward = rb.linearVelocity.sqrMagnitude > 0.0001f
                ? rb.linearVelocity.normalized
                : transform.forward;
            BoxCollider box = GetComponent<BoxCollider>();
            float tipDistance = box != null
                ? Mathf.Abs(Vector3.Dot(forward, transform.forward)) * box.size.z * Mathf.Abs(transform.lossyScale.z) * 0.5f
                : 0f;
            Ray ray = new Ray(rb.position, forward);
            if (other.Raycast(ray, out _, tipDistance + 0.05f))
            {
                BeginDelayedStick();
            }
            return;
    ...
    private void FreezeInPlace()
    {
        ...
        // A stuck bar is scenery, not another solid obstacle that can pin a creature to the wall.
        SetProjectileCollidersEnabled(false);
    ...
    private void SetProjectileCollidersEnabled(bool enabled)
    {
        Collider[] projectileColliders = GetComponentsInChildren<Collider>();
        for (int i = 0; i < projectileColliders.Length; i++)
        {
            projectileColliders[i].enabled = enabled;
        }
    }
}
